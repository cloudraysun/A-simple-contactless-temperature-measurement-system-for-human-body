...
void MLX90614_Init(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);	  				 //开漏输出 ，IO口速度为50MHz
	GPIO_SetBits(GPIOA,GPIO_Pin_7); 						 //P输出高 
}
...
{
    SCL_L;
    delay_us(1500); // greater than 1.44ms
    SCL_H;
}
void Send_Bit(unsigned char bit_out)
{
    ...
    SCL_H;                                     // Set SCK line
    delay_us(10);                           // High Level of Clock Pulse
    SCL_L;                                     // Clear SCK line
    delay_us(10);                           // Low Level of Clock Pulse
